...
export function generateSampleJudicialForm(
  type: JudicialFormData['formType'] = 'dadkhast'
): JudicialFormData {
  ...
  switch (type) {
    ...
    case 'qarardad_solh':
      return {
        ...
        evidences: ['مدارک هویتی طرفین', 'اسناد پرونده‌های مطروحه', 'رسید پرداخت مبلغ مال‌الصلح'],
    ...
    case 'shekayat':
      return {
        ...
        legalBasis:
          'ماده ۱ و ۲ قانون تشدید مجازات مرتکبین ارتشاء، اختلاس و کلاهبرداری و ماده ۶۷۴ قانون مجازات اسلامی',
    ...
    case 'layehe':
      return {
        ...
        subject:
          'لایحه دفاعیه در پاسخ به ادعای واهی خوانده و اثبات حقانیت موکل در پرونده کلاسه ...',
        evidences: [
        ...
        legalBasis:
          'مواد ۱۰، ۲۱۹، ۲۲۰، ۲۲۵ و ۲۳۰ قانون مدنی و مواد ۱۹۸ و ۵۱۹ قانون آیین دادرسی مدنی',
    ...
    default:
      return {
        ...
        subject:
          'الزام به ایفای تعهدات قراردادی، پرداخت اصل طلب به همراه خسارت تاخیر تادیه و کلیه خسارات دادرسی',
        evidences: [
...
export function renderJudicialFormHTML(form: JudicialFormData): string {
  ...
  const evidencesList = (form.evidences || [])
    .map(
      (e, idx) =>
        `<tr><td style="width: 30px; text-align: center; border: 1px solid #1e293b; padding: 4px; font-weight: bold;">${idx + 1}</td><td style="border: 1px solid #1e293b; padding: 4px;">${e}</td></tr>`
    )
    .join('');
